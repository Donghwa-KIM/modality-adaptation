...
parser.add_argument(
    "--data_path", type=str, default="../benchmark_dataset", help="path for storing the dataset"
)
parser.add_argument(
...
parser.add_argument(
    "--d_out", type=int, default=8, help="dimension of target dimension in the network (default: 4*2 for multi)"
)
...
parser.add_argument(
    "--lr", type=float, default=1e-2, help="initial learning rate (default: 1e-3)"
)
parser.add_argument(
    "--n_snapshot", type=float, default=1, help="snapshot ensemble (default: 5)"
)

parser.add_argument("--num_epochs", type=int, default=10, help="number of epochs (default: 20)")
...
def main():
    ...
    # set seed
    set_seed(args)
    label_list = ["Neutral", "Happy", "Sad", "Angry"]
    args.bert_args = torch.load(args.bert_args_path)
    
    dataset = str.lower(args.dataset.strip())
    
    train_data = benchmark_get_data(args, dataset, 'train')
    valid_data = benchmark_get_data(args, dataset, 'valid')
    test_data = benchmark_get_data(args, dataset, 'test')
    
    orig_d_t, orig_d_a, _ = train_data.get_dim()
    seq_d_t, seq_d_a , _ = train_data.get_seq_len()
    args.cls_model_name = 'en_crossModal'
    ...
    tst_loss, tst_acc, tst_f1 = test_se(args, label_list, test_dataset, models)
