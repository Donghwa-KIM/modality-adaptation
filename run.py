...
import numpy as np
from eval_metric import eval_iemocap
...
def train_modal_src(args, label_list, train_dataset, eval_dataset, model):
    ...
    for n in range(args.n_snapshot):
        ...
        for i in train_iterator:
            ...
            for step, ((_, texts, audios,_), labels, _)  in enumerate(epoch_iterator):

                model.train()

                audios, texts = list(map(lambda x: x.to(args.device), [audios, texts]))
                ...
                inputs = {'x_audio': audios,
                  'x_text': texts,
                  'a_mask': None,
                  'alpha': alphas[i]}  # alphas[i]

                senti_preds, modal_preds = model(**inputs)
                sentiLoss = loss_fct(senti_preds.view(-1,2), labels.view(-1))
            ...
            sent_val_loss, val_f1, val_acc = eval_src(args, label_list, eval_dataset, model)
            val_result = 'snapshot-{}-[{}/{}] val loss : {:.3f}, val f1 : {:.3f}'.format(n, 
                global_step, t_total, sent_val_loss, val_f1
            )
...
def eval_src(args, label_list, eval_dataset, model):
    ...
    for val_step, ((_, texts, audios,_), labels, _)  in enumerate(tqdm(eval_loader, desc="Evaluating")):
        with torch.no_grad():
            ...
            total_y_hat.append(senti_preds.view(-1, 2))
            total_y.append(labels)
            
            senti_val_loss += sentiLoss.item()
 

    f1, acc = eval_iemocap(torch.cat(total_y_hat), torch.cat(total_y))
    
    senti_val_loss /= (val_step + 1)
    
    return senti_val_loss, f1, acc
